<div>
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
        ...
        <div class="flex items-center gap-2 w-full sm:w-auto flex-wrap">
            <a href="{{ route('products.export') }}"
               class="flex items-center gap-1.5 bg-green-50 hover:bg-green-100 border border-green-200 text-green-700 px-3 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                Export
            </a>
            <button wire:click="$set('showImportModal', true)"
                    class="flex items-center gap-1.5 bg-blue-50 hover:bg-blue-100 border border-blue-200 text-blue-700 px-3 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>
                Import
            </button>
            <button wire:click="openCreate" class="flex-1 sm:flex-none bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-indigo-700 flex items-center justify-center gap-2 whitespace-nowrap transition-colors">
    ...
    <div class="block sm:hidden space-y-3">
        @forelse($products as $product)
        <div class="bg-white rounded-xl shadow-sm p-4 {{ $product->isLowStock() ? 'border-l-4 border-red-400' : 'border border-gray-100' }}">
            <div class="flex items-start justify-between gap-3">
                <div class="min-w-0 flex-1">
                    <p class="font-bold text-gray-900 text-base leading-snug">{{ $product->nama_barang }}</p>
                    <div class="flex items-center gap-1.5 mt-1 flex-wrap">
                        <span class="text-xs text-gray-500 font-mono bg-gray-100 px-1.5 py-0.5 rounded">{{ $product->kode_barang }}</span>
                        @if($product->jenis_barang)
                        <span class="text-xs text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded">{{ $product->jenis_barang }}</span>
                        @endif
                        @if($product->isLowStock())
                        <span class="text-xs text-red-600 bg-red-50 px-1.5 py-0.5 rounded font-semibold">⚠ Stok Rendah</span>
                        @endif
                    </div>
                </div>
    ...
    <div class="fixed inset-0 bg-black/60 flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
        <div class="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-md">
            <div class="flex items-center justify-between px-5 py-4 border-b">
                <div>
                    <h3 class="text-base font-bold text-gray-800">Import Produk</h3>
                    <p class="text-xs text-gray-500 mt-0.5">Upload file Excel/CSV untuk import massal</p>
                </div>
                <button wire:click="$set('showImportModal', false)" class="w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:bg-gray-100 transition-colors">
            ...
            <div class="p-5 space-y-4">
                ...
                <div class="flex items-center justify-between p-3 bg-indigo-50 rounded-xl border border-indigo-100">
                    <div class="flex items-center gap-2.5">
                        ...
                        <div>
                            <p class="text-sm font-semibold text-indigo-800">Belum punya template?</p>
                            <p class="text-xs text-indigo-600">Download template Excel dengan contoh data</p>
                        </div>
                    </div>
                    <a href="{{ route('products.import-template') }}"
                       class="flex items-center gap-1.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold px-3 py-1.5 rounded-lg transition-colors whitespace-nowrap">
                ...
                <div class="bg-gray-50 rounded-xl p-3 border border-gray-200">
                    <p class="text-xs font-bold text-gray-600 mb-2">Kolom yang dikenali (baris pertama = header):</p>
                    <div class="grid grid-cols-2 gap-1">
                        @foreach(['kode_barang *','nama_barang *','jenis_barang','kuantitas','harga_satuan','modal_awal','harga_grosir','harga_ecer','stock_minimum'] as $col)
                        <span class="text-[11px] font-mono bg-white border border-gray-200 px-2 py-0.5 rounded {{ str_contains($col,'*') ? 'text-red-600 border-red-200' : 'text-gray-600' }}">{{ $col }}</span>
                        @endforeach
                    </div>
                    <p class="text-[10px] text-gray-400 mt-2">* wajib diisi. Produk dengan kode sama akan <strong>diperbarui</strong>, bukan diduplikat.</p>
                </div>
                ...
                <div x-data="{
                    ...
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Pilih File <span class="text-red-500">*</span></label>
                    <label class="flex flex-col items-center justify-center w-full h-28 border-2 border-dashed rounded-xl cursor-pointer transition-colors"
                           :class="fileName ? 'border-indigo-400 bg-indigo-50' : 'border-gray-300 bg-gray-50 hover:bg-indigo-50 hover:border-indigo-400'">
                        <div class="flex flex-col items-center gap-1.5 text-center px-4">
                            <template x-if="!fileName">
                                <div class="flex flex-col items-center gap-1 text-gray-400">
                                    ...
                                    <p class="text-sm font-medium">Klik atau drag file ke sini</p>
                                    <p class="text-xs">.xlsx, .xls, .csv — maks 5 MB</p>
                                </div>
                            </template>
                            <template x-if="fileName">
                    ...
                    <div wire:loading wire:target="importFile" class="flex items-center gap-2 mt-1.5 text-xs text-indigo-600">
                ...
                {{-- Error list --}}
                @if($importErrors)
                <div class="bg-red-50 border border-red-200 rounded-xl p-3 max-h-32 overflow-y-auto">
                    <p class="text-xs font-bold text-red-700 mb-1">Error pada {{ count($importErrors) }} baris:</p>
                ...
                <div class="flex gap-3 pt-1">
                    ...
                    <button wire:click="importProducts" wire:loading.attr="disabled"
                        ...
                        <span wire:loading.remove wire:target="importProducts">Import Sekarang</span>
                        <span wire:loading wire:target="importProducts">Memproses...</span>
                    </button>
                </div>
            </div>
        </div>
